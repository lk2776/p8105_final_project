...
We've explored the four main types of milk—Whole, 2%, 1%, and Skim milk—quite extensively in the previous section. But what exactly sets them apart? The primary distinction lies in their nutritional content. While taste and availability also play significant roles, it's the nutrients that predominantly differentiate these types of milk and justify their coexistence in the market. In this section, we'll delve into the nutritional profiles of these four milk types and analyze their differences to understand why each has its unique place in our diets.
...
We will now perform a regression analysis on the five most recent sales ratios of the four types of milk (more about the ratios is [here](.html)) with their respective Cholesterol and Saturated Fat content. This analysis aims to determine if consumers have become more health-conscious over the years.
...
plotpr <- ggplot(df_merged, aes(x = Sales_Ratio, y = Predicted, color = Milk_Type)) +
  ...
  labs(title = "Actual vs Predicted Sales Ratio", x = "Actual Sales Ratio", y = "Predicted Sales Ratio", color = "Milk Type")
...
<ul>
 <li> Saturated fat and Sales: For the lower fat options, most of the points lie above the dashed line. This suggests that the model tends to overestimate the sales ratios for these lower fat milk types.<br>
For the higher Fat Options (Whole and 2% Milk), most of the points lie below the dashed line. This indicates that the model tends to underestimate the sales ratios for these higher fat milk types.
 </li>
 <li>Bias in Predictions: The model tends to overestimate sales ratios for lower fat milk types and underestimate them for higher fat milk types, indicating that other factors might be at play which are not accounted for by just using saturated fat as a predictor.
 </li>
 <li>Further Analysis Needed: To improve the model, it might be beneficial to include additional predictors such as price, marketing efforts, consumer preferences, or other nutritional information. Consumer preferences and dietary requirements is a crucial factor because someone may prefer taste of one over the other. It is also possible that their diet doesn't contain a lot of fat and hence, they need whole milk.
 </li>
</ul>
